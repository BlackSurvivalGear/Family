  ...
  static render() {
    ...
    this.grid.innerHTML = filtered.map(m => {
      ...
      const statusPill = m.status === 'Living'
        ? '<span class="text-[9px] font-semibold bg-emerald/10 text-emerald border border-emerald/20 px-2 py-0.5 rounded-full flex items-center gap-1"><span class="w-1 h-1 bg-emerald rounded-full"></span> Living</span>'
        : '<span class="text-[9px] font-semibold bg-slate-800 text-slate-400 border border-slate-700 px-2 py-0.5 rounded-full flex items-center gap-1"><span class="w-1 h-1 bg-slate-500 rounded-full"></span> Deceased</span>';

      return `
        <div class="glass-panel p-5 rounded-2xl border-2 ${borderClass} transition-all duration-300 flex flex-col justify-between h-[230px] shadow-sm relative group">
          <div class="flex items-start gap-3.5">
            <div class="relative shrink-0">
              <img src="${m.avatar}" alt="${m.firstName}" class="w-12 h-12 rounded-xl object-cover border-2 border-gold/25 group-hover:border-gold transition-colors">
            ...
            <div class="flex flex-col min-w-0">
              <h3 class="text-xs font-semibold text-white truncate group-hover:text-gold transition-colors">${m.firstName} ${m.lastName}</h3>
              <span class="text-[10px] text-gold italic font-light truncate">"${m.nickname || 'None'}"</span>
              <span class="text-[9px] text-slate-500 font-light truncate mt-0.5">${m.role || 'Member'}</span>
            </div>
          </div>

          <div class="flex items-center justify-between border-t border-white/5 pt-3.5 text-[10px] text-slate-400">
            ...
            <div class="flex flex-col items-end gap-1 shrink-0">
              <span class="text-[8px] font-bold uppercase bg-gold/10 text-gold px-1.5 py-0.5 rounded">Gen ${m.generation}</span>
              ${statusPill}
            </div>
          </div>

          <div class="border-t border-white/5 pt-3.5 mt-2 flex gap-2">
            <a href="member.html?id=${m.id}" class="flex-grow h-8 bg-white/5 hover:bg-gold hover:text-slate-950 font-semibold text-[10px] uppercase tracking-wider rounded-lg flex items-center justify-center gap-1.5 transition-all">
